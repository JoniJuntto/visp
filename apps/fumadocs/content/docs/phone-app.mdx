...
through Play internal testing. The browser app is a static site served by the
app host; it publishes over WHIP through the relay's WebRTC port.
...
2. Pick the camera and microphone, then go live. The stream appears in OBS as
   the corresponding relay path's Media Source.
...
[Broadcaster setup](/docs/broadcaster-setup)) while the phone reconnects.
...
- **Stream info sheet** edits the title and metadata on the provider without
  leaving the app.
- **OBS control button** starts and stops the home OBS stream and switches
  scenes through the paired [OBS plugin](/docs/obs-remote-control). The button
